#!/bin/bash
set +e
LOG=/tmp/probe-A-source.log
RESP=/tmp/probe-A-response.json
SUMM=/tmp/probe-A-summary.txt
STAT=/tmp/probe-A-status.txt

pkill -9 -f llama-server 2>/dev/null
sleep 4
echo "--- startup ---" > $STAT

cd /home/awee/code/tan_llama-tc15
nohup ./build/bin/llama-server --host 127.0.0.1 --port 18124 \
